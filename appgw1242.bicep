    ...
    sslCertificates: [
      {
        name: appGatewayFQDN
        properties: {
          keyVaultSecretId: reference(extensionResourceId('/subscriptions/${subscription().subscriptionId}/resourceGroups/${keyVaultResourceGroupName}', 'Microsoft.Resources/deployments', 'certificate')).outputs.secretUri.value
        }
      }
    ]
...
module certificate './nested_certificate.bicep' = {
  ...
  params: {
    managedIdentity: appGatewayIdentityId.id
